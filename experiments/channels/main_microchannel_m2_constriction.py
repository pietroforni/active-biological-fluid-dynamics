...
CONSTRICTION_THROAT_VALUES = np.array([0.90, 0.80, 0.70, 0.60, 0.50, 0.40, 0.30])
CONSTRICTION_LENGTH_FRACTION = 0.20
MAKE_GIFS = True 
...
CASE = {"label": "M2", "name": "middle_constriction", "geometry": "middle_constriction", "N": 3000, "a": 6.0, "phi": 0.75, "U0": 250.0}
...
def save_enstrophy_vs_constriction_size(summary, filename):
    throat = np.array([row["constriction_throat_fraction"] for row in summary])
    ...
    fig, ax = plt.subplots(figsize=(6, 4))
    ax.plot(throat, final_ens, "o-", label="final enstrophy")
    ax.errorbar(throat, mean_ens, yerr=std_ens, marker="s", capsize=4,
                label="time-avg enstrophy")
    ax.set_xlabel("constriction throat height / Ly")
    ax.set_ylabel(r"enstrophy $\Omega = \frac{1}{2}\langle \omega^2 \rangle$")
    ax.set_title(f"Enstrophy vs constriction size, Lx/Ly={CHANNEL_ASPECT_RATIO:.1f}")
    ax.grid(True, alpha=0.3)
    ax.legend()
    fig.tight_layout()
    fig.savefig(filename, dpi=400)
...
def save_vortex_dimension_vs_constriction_size(summary, filename):
    throat = np.array([row["constriction_throat_fraction"] for row in summary])
    ...
    fig, ax = plt.subplots(figsize=(6, 4))
    ax.errorbar(throat, mean_dim_local, yerr=std_dim_local, marker="o", capsize=4, label="mean vortex dimension / local height")
    ax.axhline(1.0, linestyle="--", linewidth=1.0, color="black", alpha=0.6)
    ax.set_xlabel("constriction throat height / Ly")
    ax.set_ylabel("mean vortex dimension / local channel height")
    ax.set_title(f"Vortex size vs constriction size, Lx/Ly={CHANNEL_ASPECT_RATIO:.1f}")
    ax.grid(True, alpha=0.3)
    ax.legend()
    fig.tight_layout()
    fig.savefig(filename, dpi=400)
    plt.close(fig)
    count_file = filename.with_name("vortex_count_vs_constriction_size.png")
    fig, ax = plt.subplots(figsize=(6, 4))
    ax.plot(throat, n_vortices, "o-")
    ax.set_xlabel("constriction throat height / Ly")
    ax.set_ylabel("number of detected vortices")
    ax.set_title(f"Detected vortices vs constriction size, Lx/Ly={CHANNEL_ASPECT_RATIO:.1f}")
    ax.grid(True, alpha=0.3)
    fig.tight_layout()
    fig.savefig(count_file, dpi=400)
    plt.close(fig)

def run_m2_constriction(throat_fraction):
    case = CASE.copy()
    # 'a' is now fixed from CASE, car_value is dynamic
    tag = (f"{case['label']}_{case['name']}"f"_car{CHANNEL_ASPECT_RATIO:.1f}"f"_throat{throat_fraction:.2f}").replace(".", "p")
    params = SPRParams(N=case["N"], a=case["a"], phi=case["phi"], U0=case["U0"], rc=4.0, rmin=1e-4, seed=123, dt_scale=0.002, 
                       channel_aspect_ratio=CHANNEL_ASPECT_RATIO, geometry=case["geometry"], constriction_length_fraction=CONSTRICTION_LENGTH_FRACTION,
                       constriction_throat_fraction=float(throat_fraction), n_wall_layers=1, initial_impulse_force=2.0, initial_impulse_steps=100, wall_angle=0.0,
                       )
    print(f"\nRunning {tag}")
    ...
    print(f"geometry={params.geometry}, "f"constriction_length/Lx={params.constriction_length_fraction:.3f}, "f"throat_height/Ly={params.constriction_throat_fraction:.3f}")
    ...
    print(f"mean vortex dimension/local height="f"{vortex_dim['time_mean_mean_vortex_dimension_over_local_height']:.3f}, "
          f"mean vortex dimension/Ly="f"{vortex_dim['time_mean_mean_vortex_dimension_over_Ly']:.3f}, "f"n vortices={vortex_dim['time_mean_n_vortices']:.1f}")
    ...
    row = {"channel_aspect_ratio": params.channel_aspect_ratio, "a": params.a, "final_enstrophy": ens, "mean_enstrophy": mean_ens, 
           "std_enstrophy": std_ens, "Lx": params.Lx, "Ly": params.Ly, "constriction_throat_fraction": params.constriction_throat_fraction, 
           "constriction_length_fraction": params.constriction_length_fraction}
...
def main():
    ...
    for throat_fraction in CONSTRICTION_THROAT_VALUES:
        row = run_m2_constriction(throat_fraction)
        summary.append(row)

    save_enstrophy_vs_constriction_size(
        summary,
        OUT / "enstrophy_vs_constriction_size.png",
    )

    save_vortex_dimension_vs_constriction_size(
        summary,
        OUT / "vortex_dimension_vs_constriction_size.png",
    )

    np.savez_compressed(
        OUT / "constriction_size_sweep_summary.npz",
        throat_fraction=np.array([row["constriction_throat_fraction"] for row in summary]),
        constriction_length_fraction=np.array([row["constriction_length_fraction"] for row in summary]),
    ...
    print("\nConstriction-size sweep complete.")
