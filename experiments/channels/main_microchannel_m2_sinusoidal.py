...
SINUSOIDAL_WALL_AMPLITUDE_VALUES = np.array([0.15, 0.20, 0.25, 0.35, 0.40])
SINUSOIDAL_WALL_PERIODS = 3.0
SINUSOIDAL_WALL_PHASE = 0.0
MAKE_GIFS = True
OUT = Path("spr_M2_microchannel_sinusoidal")
OUT.mkdir(exist_ok=True)
DELTA_FACTOR = 1.31
OVERLAP = 0.75
R_OVER_ELL = np.array([1.0, 2.0, 4.0])

CASE = {"label": "M2", "name": "sinusoidal_walls", "geometry": "sinusoidal_walls", "N": 3000, "a": 6.0, "phi": 0.75, "U0": 250.0}
...
def save_enstrophy_vs_sinusoidal_amplitude(summary, filename):
    amplitude = np.array([row["sinusoidal_wall_amplitude_fraction"] for row in summary])
    ...
    fig, ax = plt.subplots(figsize=(6, 4))
    ax.plot(amplitude, final_ens, "o-", label="final enstrophy")
    ax.errorbar(amplitude, mean_ens, yerr=std_ens, marker="s", capsize=4, label="time-avg enstrophy")
    ax.set_xlabel("sinusoidal wall amplitude / Ly")
    ax.set_ylabel(r"enstrophy $\Omega = \frac{1}{2}\langle \omega^2 \rangle$")
    ax.set_title(f"Enstrophy vs sinusoidal amplitude, Lx/Ly={CHANNEL_ASPECT_RATIO:.1f}")
    ax.grid(True, alpha=0.3)
    ax.legend()
    fig.tight_layout()
    fig.savefig(filename, dpi=400)
...
def save_vortex_dimension_vs_sinusoidal_amplitude(summary, filename):
    amplitude = np.array([row["sinusoidal_wall_amplitude_fraction"] for row in summary])
    mean_dim_local = np.array([row["time_mean_mean_vortex_dimension_over_local_height"] for row in summary])
    std_dim_local = np.array([row["time_std_mean_vortex_dimension_over_local_height"] for row in summary])
    n_vortices = np.array([row["time_mean_n_vortices"] for row in summary])
    fig, ax = plt.subplots(figsize=(6, 4))
    ax.errorbar(amplitude, mean_dim_local, yerr=std_dim_local, marker="o", capsize=4, label="mean vortex dimension / local height")
    ax.axhline(1.0, linestyle="--", linewidth=1.0, color="black", alpha=0.6)
    ax.set_xlabel("sinusoidal wall amplitude / Ly")
    ax.set_ylabel("mean vortex dimension / local channel height")
    ax.set_title(f"Vortex size vs sinusoidal amplitude, Lx/Ly={CHANNEL_ASPECT_RATIO:.1f}")
    ax.grid(True, alpha=0.3)
    ax.legend()
    fig.tight_layout()
    fig.savefig(filename, dpi=400)
    plt.close(fig)
    count_file = filename.with_name("vortex_count_vs_sinusoidal_amplitude.png")
    fig, ax = plt.subplots(figsize=(6, 4))
    ax.plot(amplitude, n_vortices, "o-")
    ax.set_xlabel("sinusoidal wall amplitude / Ly")
    ax.set_ylabel("number of detected vortices")
    ax.set_title(f"Detected vortices vs sinusoidal amplitude, Lx/Ly={CHANNEL_ASPECT_RATIO:.1f}")
    ax.grid(True, alpha=0.3)
    fig.tight_layout()
    fig.savefig(count_file, dpi=400)
    plt.close(fig)

def run_m2_sinusoidal(amplitude_fraction):
    case = CASE.copy()
    tag = (f"{case['label']}_{case['name']}"
           f"_car{CHANNEL_ASPECT_RATIO:.1f}"
           f"_amp{amplitude_fraction:.2f}"
           f"_periods{SINUSOIDAL_WALL_PERIODS:.1f}").replace(".", "p")
    params = SPRParams(N=case["N"], a=case["a"], phi=case["phi"], U0=case["U0"], rc=4.0, rmin=1e-4, seed=123, dt_scale=0.002,
                       channel_aspect_ratio=CHANNEL_ASPECT_RATIO, geometry=case["geometry"],
                       sinusoidal_wall_amplitude_fraction=float(amplitude_fraction),
                       sinusoidal_wall_periods=float(SINUSOIDAL_WALL_PERIODS),
                       sinusoidal_wall_phase=float(SINUSOIDAL_WALL_PHASE),
                       n_wall_layers=1, initial_impulse_force=2.0, initial_impulse_steps=30000, wall_angle=0.0)
    print(f"\nRunning {tag}")
    ...
    print(f"geometry={params.geometry}, "
          f"wall_amplitude/Ly={params.sinusoidal_wall_amplitude_fraction:.3f}, "
          f"periods={params.sinusoidal_wall_periods:.3f}, "
          f"phase={params.sinusoidal_wall_phase:.3f}")
    ...
    print(f"mean vortex dimension/local height="
          f"{vortex_dim['time_mean_mean_vortex_dimension_over_local_height']:.3f}, "
          f"mean vortex dimension/Ly="
          f"{vortex_dim['time_mean_mean_vortex_dimension_over_Ly']:.3f}, "
          f"n vortices={vortex_dim['time_mean_n_vortices']:.1f}")

    save_rods_snapshot(pos, theta, wall_mask, params, OUT / f"{tag}_rods.png",
                       title=f"M2 sinusoidal rods: Lx/Ly={params.channel_aspect_ratio:.1f}, a={params.a}")
    save_vorticity_plot(omega, params, OUT / f"{tag}_vorticity.png",
                        title=f"M2 sinusoidal vorticity: Lx/Ly={params.channel_aspect_ratio:.1f}, a={params.a}")
    save_vorticity_y_profile(omega, params, OUT / f"{tag}_vorticity_y_profile.png",
                             title=f"M2 sinusoidal vorticity profile: Lx/Ly={params.channel_aspect_ratio:.1f}")
    save_density_profile(pos, wall_mask, params, OUT / f"{tag}_density_profile.png",
                         title=f"M2 sinusoidal density profile: Lx/Ly={params.channel_aspect_ratio:.1f}")
    save_histogram(vel, wall_mask, OUT / f"{tag}_histogram.png",
                   title=f"M2 sinusoidal velocity histogram: Lx/Ly={params.channel_aspect_ratio:.1f}")
    save_channel_profiles(pos, vel, wall_mask, params, OUT / f"{tag}_velocity_profile_final.png",
                          title=f"M2 sinusoidal velocity profile: Lx/Ly={params.channel_aspect_ratio:.1f}", n_bins=20)
    save_x_increment_plot(pdfs, params, OUT / f"{tag}_x_increment.png",
                          title=f"M2 sinusoidal velocity-increment PDF: Lx/Ly={params.channel_aspect_ratio:.1f}")
    ...
    row = {"channel_aspect_ratio": params.channel_aspect_ratio, "a": params.a, "final_enstrophy": ens,
           "mean_enstrophy": mean_ens, "std_enstrophy": std_ens, "Lx": params.Lx, "Ly": params.Ly,
           "sinusoidal_wall_amplitude_fraction": params.sinusoidal_wall_amplitude_fraction,
           "sinusoidal_wall_periods": params.sinusoidal_wall_periods,
           "sinusoidal_wall_phase": params.sinusoidal_wall_phase}
...
def main():
    ...
    for amplitude_fraction in SINUSOIDAL_WALL_AMPLITUDE_VALUES:
        row = run_m2_sinusoidal(amplitude_fraction)
        summary.append(row)

    save_enstrophy_vs_sinusoidal_amplitude(
        summary,
        OUT / "enstrophy_vs_sinusoidal_amplitude.png",
    )

    save_vortex_dimension_vs_sinusoidal_amplitude(
        summary,
        OUT / "vortex_dimension_vs_sinusoidal_amplitude.png",
    )

    np.savez_compressed(
        OUT / "sinusoidal_amplitude_sweep_summary.npz",
        amplitude_fraction=np.array([row["sinusoidal_wall_amplitude_fraction"] for row in summary]),
        wall_periods=np.array([row["sinusoidal_wall_periods"] for row in summary]),
        wall_phase=np.array([row["sinusoidal_wall_phase"] for row in summary]),
        ...
        mean_vortex_dimension_over_Ly=np.array([
            row["time_mean_mean_vortex_dimension_over_Ly"] for row in summary
        ]),
        std_vortex_dimension_over_Ly=np.array([
            row["time_std_mean_vortex_dimension_over_Ly"] for row in summary
        ]),
        mean_vortex_dimension_over_local_height=np.array([
            row["time_mean_mean_vortex_dimension_over_local_height"] for row in summary
        ]),
        std_vortex_dimension_over_local_height=np.array([
            row["time_std_mean_vortex_dimension_over_local_height"] for row in summary
        ]),
        mean_n_vortices=np.array([
            row["time_mean_n_vortices"] for row in summary
        ]),
    )

    print("\nSinusoidal-amplitude sweep complete.")
